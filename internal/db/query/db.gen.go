...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	q := Queries{db: db}
	var err error
	if q.getJumpgatesInSystemStmt, err = db.PrepareContext(ctx, getJumpgatesInSystem); err != nil {
		return nil, fmt.Errorf("error preparing query GetJumpgatesInSystem: %w", err)
	}
	...
	if q.getWaypointsByTypeStmt, err = db.PrepareContext(ctx, getWaypointsByType); err != nil {
		return nil, fmt.Errorf("error preparing query GetWaypointsByType: %w", err)
	}
	...
	if q.hasJumpgateRowsStmt, err = db.PrepareContext(ctx, hasJumpgateRows); err != nil {
		return nil, fmt.Errorf("error preparing query HasJumpgateRows: %w", err)
	}
...
func (q *Queries) Close() error {
	var err error
	if q.getJumpgatesInSystemStmt != nil {
		if cerr := q.getJumpgatesInSystemStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getJumpgatesInSystemStmt: %w", cerr)
		}
	}
	...
	if q.getWaypointsByTypeStmt != nil {
		if cerr := q.getWaypointsByTypeStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getWaypointsByTypeStmt: %w", cerr)
		}
	}
	...
	if q.hasJumpgateRowsStmt != nil {
		if cerr := q.hasJumpgateRowsStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing hasJumpgateRowsStmt: %w", cerr)
		}
	}
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		db:                        tx,
		tx:                        tx,
		getJumpgatesInSystemStmt:  q.getJumpgatesInSystemStmt,
		...
		getWaypointsByTypeStmt:    q.getWaypointsByTypeStmt,
		getWaypointsForSystemStmt: q.getWaypointsForSystemStmt,
		hasJumpgateRowsStmt:       q.hasJumpgateRowsStmt,
