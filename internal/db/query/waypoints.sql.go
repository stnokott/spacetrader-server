...
const getWaypointsByType = `-- name: GetWaypointsByType :many
SELECT symbol, system, orbits, x, y, type FROM waypoints
WHERE type = ?1
`

func (q *Queries) GetWaypointsByType(ctx context.Context, type_ string) ([]Waypoint, error) {
	rows, err := q.query(ctx, q.getWaypointsByTypeStmt, getWaypointsByType, type_)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
